...
export const CollapsibleLiveCode = ({children, title = "💻 View Source Code", defaultOpen = false}) => {
...
export const BotAnalytics = () => {
  ...
  return (
    <div style={{
      ...
      <h3 style={{color: '#667eea', marginTop: 0}}>📈 Bot Analytics Dashboard</h3>
      ...
      <div style={{
        ...
        {[
          { label: 'Total Commands', value: stats.totalCommands, icon: '💬', color: '#667eea' },
          { label: 'Registrations', value: stats.registrations, icon: '✍️', color: '#25c2a0' },
          { label: 'Releases', value: stats.releases, icon: '🔓', color: '#ff6b6b' },
          { label: 'Broadcasts', value: stats.broadcasts, icon: '📢', color: '#4ecdc4' },
          { label: 'Active Users', value: stats.activeUsers, icon: '👥', color: '#feca57' },
          { label: 'Avg Response', value: stats.avgResponseTime, icon: '⚡', color: '#a29bfe' },
        ].map((stat, idx) => (
          <div key={idx} style={{
            padding: '15px',
            backgroundColor: '#f8f9fa',
...
export const CommandDemo = () => {
  ...
  const commands = {
    'testbed': '📊 Displaying main menu with options:\n• Check Status\n• Register Testbed\n• Release Testbed\n• Broadcast',
    'help': '❓ Available commands:\n• testbed - Main menu\n• help - Show this help',
    'status': '✅ Testbed Status:\nTestbed-1: Available\nTestbed-2: In Use (John Doe)\nIaC1: Available',
  };

  const handleCommand = () => {
    const result = commands[command.toLowerCase()] || '❌ Unknown command. Try "help"';
    setOutput(result);
  };
  ...
  return (
    <div style={{
      ...
      <h3 style={{color: '#667eea', marginTop: 0}}>💬 Bot Command Simulator</h3>
...
<InfoCard title="Why WebSocket?" icon="🔌" color="#25c2a0">
...
<Tabs>
  <TabItem value="monitoring" label="📊 Monitoring" default>
...
```
  ...
  <TabItem value="registration" label="✍️ Registration">
...
6. Bot updates database and confirms
  ...
  <TabItem value="broadcasting" label="📢 Broadcasting">
...
export const botImages = [
  {
    ...
    alt: 'Main Menu',
    caption: '🏠 Main Menu - Entry point with all bot functions'
  },
  {
    src: '/img/webex-bot/status.png',
    alt: 'Status View',
    caption: '📊 Status Dashboard - Real-time testbed availability'
  },
  {
    src: '/img/webex-bot/Register.png',
    alt: 'Registration Form',
    caption: '✍️ Registration Form - Interactive booking interface'
  },
  {
    src: '/img/webex-bot/success_register.png',
    alt: 'Success Message',
    caption: '✅ Success Confirmation - Registration complete'
  },
  {
    src: '/img/webex-bot/broadcast_form.png',
    alt: 'Broadcast Selection',
    caption: '📢 Broadcast Form - Select rooms for notifications'
  },
  {
    src: '/img/webex-bot/broadcastmessage.png',
    alt: 'Broadcast Message',
    caption: '💬 Broadcast Message - Notification sent to teams'
  },
  {
    src: '/img/webex-bot/release_form.png',
    alt: 'Release Form',
    caption: '🔓 Release Form - Free up testbed resources'
  },
  {
    src: '/img/webex-bot/help_start.png',
    alt: 'Help Command',
    caption: '❓ Help Menu - Available commands and guidance'
  },
  {
    src: '/img/webex-bot/feedback.png',
    alt: 'Feedback',
    caption: '💭 User Feedback - Bot interaction example'
  },
];
...
<InfoCard title="Requirements Checklist" icon="✅" color="#4ecdc4">
...
3. Navigate to **My Apps** → **Create a New App**
...
<InfoCard title="🔒 Security Warning" icon="⚠️" color="#ff6b6b">
...
- Secret management systems (AWS Secrets Manager, Azure Key Vault)
  ...
  <TabItem value="api" label="🔧 API Method">
...
<InfoCard title="🔍 Finding Room IDs" icon="🆔" color="#667eea">
...
if __name__ == "__main__":
    print("🚀 Starting Testbed Monitor Bot...")
    print(f"📝 Bot Name: {Config.BOT_NAME}")
    print(f"🔧 Approved Rooms: {len(Config.APPROVED_ROOMS)}")
    bot.run()
...
<Tabs>
  <TabItem value="mainmenu" label="🏠 Main Menu" default>
...
def create_main_menu():
    card = AdaptiveCard(
        ...
        actions=[
            Submit(title="📊 Check Status", data={"action": "status"}),
            Submit(title="✍️ Register Testbed", data={"action": "register"}),
            Submit(title="🔓 Release Testbed", data={"action": "release"}),
            Submit(title="📢 Broadcast", data={"action": "broadcast"}),
        ]
    )
    return card.to_dict()
```
  ...
  <TabItem value="register" label="✍️ Registration Form">
...
def create_registration_form(available_testbeds):
    ...
    card = AdaptiveCard(
        ...
        actions=[
            Submit(title="✅ Register", data={"callback": "register_submit"}),
            Submit(title="❌ Cancel", data={"callback": "cancel"})
        ]
    )
    return card.to_dict()
```
  ...
  <TabItem value="status" label="📊 Status Display">
...
def create_status_card(testbeds):
    ...
    for tb in testbeds:
        ...
        status_text = "✅ Available" if tb["status"] == "available" else "🔴 In Use"
    ...
    card = AdaptiveCard(
        body=[
            TextBlock("📊 Testbed Status", size="Large", weight="Bolder"),
            *containers
        ],
        actions=[
            Submit(title="🔄 Refresh", data={"action": "refresh_status"})
        ]
    )
    return card.to_dict()
```
...
<InfoCard title="📢 Multi-Room Broadcasting" icon="📡" color="#25c2a0">
...
<InfoCard title="Development Tips" icon="💡" color="#667eea">
...
- Monitor `testbed_monitor.json` changes in real-time
  ...
  <TabItem value="linux" label="🐧 Linux Server">
...
```
  ...
  <TabItem value="cloud" label="☁️ Cloud Platforms">
...
<InfoCard title="🌐 Corporate Proxy Configuration" icon="🔒" color="#ff6b6b">
...
<Tabs>
  <TabItem value="json" label="📄 JSON (Current)" default>
...
- No built-in transactions
  ...
  <TabItem value="sqlite" label="🗄️ SQLite">
...
<InfoCard title="Common Issues & Solutions" icon="🔧" color="#4ecdc4">
...
<FeatureGrid>
  <InfoCard title="Security" icon="🔒" color="#ff6b6b">
...
- Implement rate limiting
  ...
  <InfoCard title="Error Handling" icon="🛡️" color="#667eea">
...
- Automatic retry logic
  ...
  <InfoCard title="Performance" icon="⚡" color="#25c2a0">
...
- Connection pooling
  ...
  <InfoCard title="Monitoring" icon="📊" color="#4ecdc4">
...
<Tabs>
  <TabItem value="features" label="🎯 Feature Ideas" default>
...
def notify_slack(testbed_info):
    ...
    client.chat_postMessage(
        ...
        text=f"🔴 {testbed_info['testbed_name']} now in use by {testbed_info['user_name']}"
    )
```
...
<div style={{
  ...
  {[
    { icon: '🔌', title: 'WebSocket Magic', desc: 'No webhooks, no public IP, works behind firewalls' },
    { icon: '🎴', title: 'Rich UI', desc: 'Adaptive Cards provide interactive, beautiful interfaces' },
    { icon: '📊', title: 'Real-time', desc: 'Instant updates and bidirectional communication' },
    { icon: '🚀', title: 'Scalable', desc: 'Start simple, scale to enterprise-grade deployments' },
    { icon: '🔒', title: 'Secure', desc: 'Built-in authentication and authorization' },
    { icon: '🛠️', title: 'Extensible', desc: 'Easy to add new features and integrations' },
  ].map((item, idx) => (
    <div key={idx} style={{
      padding: '20px',
      backgroundColor: 'var(--ifm-background-surface-color)',
...
<div style={{textAlign: 'center', color: '#666', fontSize: '0.9rem', marginTop: '50px'}}>
  <p>
    <strong>Built with ❤️ using Webex, Python, and WebSockets</strong>
  </p>
  <p>
